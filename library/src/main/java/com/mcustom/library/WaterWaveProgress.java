...
public class WaterWaveProgress extends View {
    ...
    // 圆环颜色
    private int mRingColor = 0xFF33B5E5;
    // 圆环背景颜色
    private int mRingBgColor = 0xFFBEBEBE;
    //水波颜色
    private int mWaterColor = 0XFF4BBDFE;
    // 水波背景色
    private int mWaterBgColor = 0xFFDDDDDD;
    //进度百分比字体大小
    ...
    public WaterWaveProgress(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.WaterWaveProgress, defStyleAttr, 0);
            mRingWidth = typedArray.getDimensionPixelOffset(R.styleable.WaterWaveProgress_progressWidth, 0);
            mRingColor = typedArray.getColor(R.styleable.WaterWaveProgress_progressColor, mRingColor);
            mRingBgColor = typedArray.getColor(R.styleable.WaterWaveProgress_progressBgColor, mRingBgColor);
            mWaterColor = typedArray.getColor(R.styleable.WaterWaveProgress_waterWaveColor, mWaterColor);
            mWaterBgColor = typedArray.getColor(R.styleable.WaterWaveProgress_waterWaveBgColor, mWaterBgColor);
            mProgress2WaterWidth = typedArray.getDimensionPixelOffset(R.styleable.WaterWaveProgress_progress2WaterWidth, 0);
            mShowProgress = typedArray.getBoolean(R.styleable.WaterWaveProgress_showProgress, true);
            mShowNumerical = typedArray.getBoolean(R.styleable.WaterWaveProgress_showNumerical, true);
            mFontSize = typedArray.getDimensionPixelOffset(R.styleable.WaterWaveProgress_fontSize, 0);
            mTextColor = typedArray.getColor(R.styleable.WaterWaveProgress_android_textColor, 0xFFFFFFFF);
            mProgress = typedArray.getInteger(R.styleable.WaterWaveProgress_progress, 15);
            mMaxProgress = typedArray.getInteger(R.styleable.WaterWaveProgress_maxProgress, 100);
            typedArray.recycle();
        }
        init(context);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        { // 重新设置进度条的宽度和水波与进度条的距离,,至于为什么写在这,我脑袋抽了可以不
            ...
            mProgress2WaterWidth = mProgress2WaterWidth == 0 ? mRingWidth * 0.6f : mProgress2WaterWidth;
